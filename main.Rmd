...
2. How vaccination and social distancing rules affect the spread of covid?
...
```
As you can see that these are the countries that we saw present almost all the time in previous bar chart ranking animation.
...
Now lets add daily vaccinated dataset to it.
...
data_vac <- 
    ...
    mutate(text = paste0("New Vaccinations: ", comma(daily_vaccinations),
                         "\nNew Cases: ", comma(daily_new_cases),
                         "\nCountry: ", country, 
                         "\nDate: ", date))

p <- 
    ggplot(data_vac) +
    geom_line(aes(date, daily_new_cases, group = country, color = country, text = text)) +
