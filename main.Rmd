...
All code block within this section is essential for the rest of the plotting sections.
Remaining plotting code blocks are independent of each other, can be ran individually.
...
Auto run for this block is disabled with `eval=FALSE`, run it manually to enforce it.
...
This is derived from previous map, as I notice that most of the cases are from big countries. 

```{r fig.align="center", fig.width = 10, fig.height = 6, eval=TRUE}
...
```
- Double Click Legend to filter
- Hover to view details

2. How vaccination affect the spread of covid?

This derived from previous bar ranking chart, there are some huge spike for active cases in some country.
Lets take a closer look using daily new cases.
...
```
As you can see that most of these countries can be found in the previous top 10 bar chart ranking animation.

Lets take an even closer look to their daily new cases.
...
There's a huge spike in india, I wonder how this relates to vaccination number.
Now lets join daily vaccinated dataset to it.
